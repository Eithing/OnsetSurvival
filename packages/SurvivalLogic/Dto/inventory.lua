...
function GetPlayerInventory(id)
    local query = mariadb_prepare(sql, "SELECT inventory.id, items.nom, items.value, items.maxStack, items.isstackable, items.poids, items.type, items.imageId, items.modelId, inventory.compteId, inventory.itemId, inventory.itemCount, inventory.var FROM items INNER JOIN inventory ON items.id = inventory.itemId WHERE inventory.compteId = '?';", id)
